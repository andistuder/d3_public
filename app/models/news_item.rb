class NewsItem < ActiveRecord::Base
  ...
  def self.find_latest_product_updates
    NewsItem.select{|h| h.categories.include?(Category.find_by_name('Product updates'))}.take(D3::Application::NEWS_INITIAL_LOAD)
  end
  ...
  def self.find_next_product_update(page = 1)

    offset = page.to_i <= 1 ?  0 : D3::Application::NEWS_PAGE_SIZE * (page.to_i - 1)
    offset += D3::Application::NEWS_INITIAL_LOAD

    NewsItem.select{|h| h.categories.include?(Category.find_by_name('Product updates'))}.drop(offset).take(D3::Application::NEWS_PAGE_SIZE)
  end
